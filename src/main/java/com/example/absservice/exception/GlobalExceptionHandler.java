...
public class GlobalExceptionHandler {
    ...
    private static final int ERROR_ACCESS_DENIED_CODE = 2006;

    private static final int ERROR_VALIDATION_NULL_CODE = 2001;

    private static final int ERROR_VALIDATION_PATTERN_CODE = 2002;
    ...
    public ResponseEntity<ApiErrorResponseDto> handleAccessDeniedException(AccessDeniedException e) {
        ...
        apiErrorResponseDto.setMessage(ERROR_ACCESS_DENIED);
        apiErrorResponseDto.setCode(ERROR_ACCESS_DENIED_CODE);
        return ResponseEntity.status(status).body(apiErrorResponseDto);
    }
    ...
    private ApiErrorResponseDto getValidationErrorResponse(BindingResult bindingResult) {
        ...
        var nullFields = errors.stream()
            .filter(e -> e.getCode().equals("NotNull"))
            .toList();
